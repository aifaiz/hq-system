<div x-data class="space-y-6 overflow-hidden rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
    <a href="#" class="overflow-hidden rounded">
        @if($image)
            <img class="mx-auto w-full h-52 rounded-lg object-cover" src="/{{$image}}" alt="{{$name}}" />
        @else
    ...
    <div class="mt-6 flex items-center gap-2.5">
        ...
        @if($enableOrder == 'YES')
        <button x-on:click="$store.cart.addItem({id:'{{$productid}}',image: '/{{$image}}',name:'{{$name}}',price: '{{$price}}',qty:1})" type="button" class="inline-flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium  text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
        ...
        @else
        <button type="button" class="inline-flex w-full items-center justify-center rounded-lg bg-gray-400 px-5 py-2.5 text-sm font-medium  text-white hover:bg-gray-500 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">
            Unavailable
        </button>
        @endif
    </div>
